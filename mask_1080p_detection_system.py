#!/usr/bin/env python3
"""
基于mask.png的1080p颜色变化检测系统
使用1920x1080分辨率，只在mask白色区域内检测红色光点
"""
...
class CameraMaskState:
    """摄像头mask检测状态"""
    camera_id: int
    red_points: List[MaskRedPoint] = None
...
class Mask1080pDetectionSystem:
    """基于mask.png的1080p检测系统"""
    ...
    def __init__(self, config_file: str = "config.yaml", mask_file: str = "mask.png"):
        ...
        self.logger.info(f"基于mask.png的1080p检测系统初始化完成，mask点数: {len(self.mask_points_coords)}")
    ...
    def _load_mask(self) -> bool:
        ...
        self.logger.info(f"检测到 {len(self.mask_points_coords)} 个白色mask点")
    ...
    def _extract_red_points_in_mask(self, frame: np.ndarray) -> List[MaskRedPoint]:
        """提取mask区域内的红色光点"""
        red_points = []
    ...
    def _compare_red_points(self, baseline_points: List[MaskRedPoint], frame: np.ndarray) -> Tuple[int, int]:
        """比较当前帧与基线的红色光点变化"""
        # 获取当前帧中mask区域的红色点
        current_red_points = self._extract_red_points_in_mask(frame)
        current_red_coords = {(p.x, p.y) for p in current_red_points}
        baseline_red_coords = {(p.x, p.y) for p in baseline_points}
        
        # 计算变化
        disappeared_points = baseline_red_coords - current_red_coords  # 基线有但现在没有的
        appeared_points = current_red_coords - baseline_red_coords     # 基线没有但现在有的
        
        total_changes = len(disappeared_points) + len(appeared_points)
    ...
    def capture_baseline(self) -> None:
        """捕获基线数据 - 只采集mask白色区域内的红色光点"""
        self.logger.info("开始捕获mask区域内红色光点基线数据...")
        ...
        with self.detection_lock:
            for camera_id, cap in enumerate(self.cameras):
                ...
                try:
                    ...
                    # 提取mask区域内的红色光点
                    red_points = self._extract_red_points_in_mask(frame)
                    
                    # 设置基线
                    state = self.camera_states[camera_id]
                    state.red_points = red_points
                    state.baseline_red_count = len(red_points)
                    state.current_red_count = len(red_points)
                    state.baseline_established = True
                    state.last_detection_time = time.time()
                    
                    self.logger.info(f"摄像头 {camera_id} 基线设置: mask区域内红色光点数={len(red_points)}")
    ...
    def detect_and_compare(self) -> None:
        ...
        with self.detection_lock:
            for camera_id, cap in enumerate(self.cameras):
                ...
                # 检查是否需要检测
                if not state.baseline_established or state.red_points is None:
                    continue
                ...
                try:
                    ...
                    # 比较红色光点变化
                    current_count, change_count = self._compare_red_points(state.red_points, frame)
                    ...
                    # 检查是否有显著变化
                    if change_count > 0:
                        self.logger.info(f"摄像头 {camera_id} 检测到红色光点变化: "
                    ...
                    if state.detection_count % 25 == 0:  # 每5秒输出一次
                        self.logger.info(f"摄像头 {camera_id} 状态: "
                                       f"基线红色点数={state.baseline_red_count}, "
                                       f"当前红色点数={current_count}, "
    ...
    def _trigger_mqtt_message(self, camera_id: int, change_count: int) -> None:
        ...
        try:
            if self.mqtt_client and self.mqtt_client.client:
                ...
                if result.rc == 0:
                    self.logger.info(f"摄像头 {camera_id} 触发MQTT消息成功 (红色光点变化: {change_count})")
                else:
    ...
    def detection_loop(self) -> None:
        """检测循环"""
        self.logger.info("开始mask区域红色光点检测循环")
    ...
    def start(self) -> bool:
        """启动系统"""
        try:
            self.logger.info("启动基于mask.png的1080p检测系统")
            ...
            self.logger.info("基于mask.png的1080p检测系统启动成功")
            return True
    ...
    def stop(self) -> None:
        """停止系统"""
        try:
            self.logger.info("停止基于mask.png的检测系统")
            ...
            self.logger.info("基于mask.png的检测系统已停止")
...
def setup_logging():
    ...
    file_handler = logging.FileHandler('mask_1080p_detection.log', encoding='utf-8')
...
def main():
    """主函数"""
    print("=== 基于mask.png的1080p红色光点检测系统 ===")
    print("使用mask.png定义检测区域")
    print("只在mask白色区域内检测红色光点变化")
    print("按 Ctrl+C 退出")
    print()
    ...
    try:
        # 创建检测系统
        system = Mask1080pDetectionSystem(mask_file=mask_file)
            ...
            for camera_id, state in status['camera_states'].items():
                if state['baseline_established']:
                    logger.info(f"摄像头 {camera_id}: "
                               f"基线红色点数={state['baseline_red_count']}, "
                               f"当前红色点数={state['current_red_count']}, "
                               f"检测次数={state['detection_count']}")
